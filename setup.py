...
def main():
    ...
    shutil.copytree(Path.home()/"Downloads"/"OhioT1DM"/"processed_data"/"2020data", base_dir/"2020data", dirs_exist_ok=True)
    shutil.copytree(Path.home()/"Downloads"/"OhioT1DM"/"processed_data"/"2018data", base_dir/"2018data", dirs_exist_ok=True)
    # 5. Copy pretrained models
    """Run a shell command and print output in real time."""
    print("\n>>> Running: cp -r ~/Downloads/OhioT1DM/models/PRETRAINS .")
    shutil.copytree(Path.home()/"Downloads"/"OhioT1DM"/"models"/"PRETRAINS", base_dir/"PRETRAINS", dirs_exist_ok=True)
